...
Hosted with GitHub Actions, it runs daily on staggered early attempts from `12:23 UTC` through `14:53 UTC`, and:
...
- The scheduled workflow times are fixed in UTC. During Central Daylight Time, the workflow starts trying every 30 minutes from `7:23 AM` through `9:53 AM`.
- GitHub scheduled workflows can start later than their cron time or occasionally miss a run. Multiple early off-hour attempts give the bot several chances while still aiming for a morning post.
